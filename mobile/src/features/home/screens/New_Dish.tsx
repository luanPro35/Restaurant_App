...
export default function New_Dish({ navigation }: { navigation: any }) {
    ...
    const fetchNewDishes = async () => {
      try {
        ...
        if (allProducts.length > 0) {
          const shuffled = seededShuffle(allProducts, getDailySeed() + "_new");
          setDishes(shuffled.slice(0, 8));
        }
      } catch (err) {
        console.error("New_Dish fetch error:", err);
      } finally {
        setLoading(false);
      }
    };
    ...
    <View className="py-2">
      <View className="px-4 mb-4 flex-row items-center justify-between">
        <View className="flex-row items-center">
          ...
          <View>
            <Text className="text-2xl font-black text-gray-900 tracking-tighter uppercase">Món Mới</Text>
            <Text className="text-[#E07B39] text-[10px] font-bold uppercase tracking-[2px] mt-0.5">Khám phá hương vị mới</Text>
          </View>
        </View>
        <TouchableOpacity>
          <Text className="text-[#E07B39] text-xs font-semibold">Tất cả</Text>
        </TouchableOpacity>
      </View>
